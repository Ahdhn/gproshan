...
ptp_out_t::ptp_out_t(distance_t* const& d, index_t* const& c)
    : dist(d), clusters(c)
{
}
...
#pragma omp parallel for
    ...
    for (index_t he = 0; he < mesh->n_half_edges(); he++)
        if (inv[mesh->vt(he)] != NIL && inv[mesh->vt(prev(he))] != NIL &&
            inv[mesh->vt(next(he))] != NIL)
            F.push_back(inv[mesh->vt(he)]);

    return new che(V.data(), toplesets.limits.back(), F.data(),
                   F.size() / che::P);
}

void parallel_toplesets_propagation_coalescence_cpu(
    ...
    const toplesets_t&     toplesets)
{
    ...
    // ------------------------------------------------------
    distance_t* pdist[2] = {new distance_t[mesh->n_vertices()],
                            new distance_t[mesh->n_vertices()]};
...
#pragma omp parallel for
    ...
    for (index_t i = 0; i < sources.size(); i++) {
        ...
        if (ptp_out.clusters)
            ptp_out.clusters[inv[sources[i]]] = i + 1;
    }
        ...
        for (index_t v = start; v < end; v++) {
            ...
            for_star(he, mesh, v)
            {
                ...
                if (p < pdist[!d][v]) {
                    ...
                    if (ptp_out.clusters)
                        ptp_out.clusters[v] =
                            ptp_out.clusters[mesh->vt(prev(he))] != NIL ?
                                ptp_out.clusters[mesh->vt(prev(he))] :
                                ptp_out.clusters[mesh->vt(next(he))];
                }
            }
        }
...
#pragma omp parallel for
    ...
    for (index_t i = 0; i < sources.size(); i++) {
        ...
        if (ptp_out.clusters)
            ptp_out.clusters[sources[i]] = i + 1;
    }
...
#pragma omp parallel for reduction(+ : count)
    ...
    if (ptp_out.dist != pdist[!d]) {
        memcpy(ptp_out.dist, pdist[!d],
               mesh->n_vertices() * sizeof(distance_t));
        delete[] pdist[!d];
    } else
        delete[] pdist[d];
}
...
distance_t update_step(che* mesh, const distance_t* dist, const index_t& he)
{
    ...
    distance_t p =
        (delta + sqrt(dis)) / (Q[0][0] + Q[0][1] + Q[1][0] + Q[1][1]);

    distance_t tp[2];
    tp[0] = t[0] - p;
    tp[1] = t[1] - p;

    vertex n(tp[0] * (X[0][0] * Q[0][0] + X[1][0] * Q[1][0]) +
