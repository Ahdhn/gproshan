...
__host__ __device__ index_t cu_trig(index_t he)
{
    if (he == NIL)
        return NIL;
    return he / che::P;
...
__host__ __device__ index_t cu_next(index_t he)
{
    if (he == NIL)
        return NIL;
...
__host__ __device__ index_t cu_prev(index_t he)
{
    if (he == NIL)
        return NIL;
...
void cuda_create_CHE(CHE* h_che, CHE*& dd_che, CHE*& d_che)
{
    ...
    cudaMemcpy(dd_che->GT, h_che->GT, sizeof(vertex_cu) * h_che->n_vertices,
               cudaMemcpyHostToDevice);

    cudaMalloc(&dd_che->VT, sizeof(index_t) * h_che->n_half_edges);
    cudaMemcpy(dd_che->VT, h_che->VT, sizeof(index_t) * h_che->n_half_edges,
               cudaMemcpyHostToDevice);

    cudaMalloc(&dd_che->OT, sizeof(index_t) * h_che->n_half_edges);
    cudaMemcpy(dd_che->OT, h_che->OT, sizeof(index_t) * h_che->n_half_edges,
               cudaMemcpyHostToDevice);

    cudaMalloc(&dd_che->EVT, sizeof(index_t) * h_che->n_vertices);
    cudaMemcpy(dd_che->EVT, h_che->EVT, sizeof(index_t) * h_che->n_vertices,
               cudaMemcpyHostToDevice);
