                                          ...
                                          const toplesets_t&     toplesets)
{
    ...
    if (ptp_out.clusters) {
        ...
        d = run_ptp_gpu(d_mesh, h_mesh->n_vertices, h_dist, d_dist, sources,
                        toplesets.limits, toplesets.index, d_sorted, d_error,
                        h_clusters, d_clusters);
        cudaMemcpy(h_clusters, d_clusters[d],
                   sizeof(index_t) * h_mesh->n_vertices,
    ...
    cudaMemcpy(h_dist, d_dist[d], sizeof(distance_t) * h_mesh->n_vertices,
               cudaMemcpyDeviceToHost);
...
index_t run_ptp_gpu(CHE*                   d_mesh,
                    const index_t&         n_vertices,
                    distance_t*            h_dist,
                    distance_t**           d_dist,
                    const vector<index_t>& sources,
                    const vector<index_t>& limits,
                    const index_t*         h_sorted,
                    index_t*               d_sorted,
                    distance_t*            d_error,
                    index_t*               h_clusters,
                    index_t**              d_clusters)
...
#pragma omp parallel for
    ...
    cudaMemcpy(d_dist[0], h_dist, sizeof(distance_t) * n_vertices,
               cudaMemcpyHostToDevice);
    cudaMemcpy(d_dist[1], h_dist, sizeof(distance_t) * n_vertices,
               cudaMemcpyHostToDevice);
    cudaMemcpy(d_sorted, h_sorted, sizeof(index_t) * n_vertices,
               cudaMemcpyHostToDevice);
    ...
    if (h_clusters) {
        ...
        cudaMemcpy(d_clusters[0], h_clusters, sizeof(index_t) * n_vertices,
                   cudaMemcpyHostToDevice);
        cudaMemcpy(d_clusters[1], h_clusters, sizeof(index_t) * n_vertices,
                   cudaMemcpyHostToDevice);
    }
    ...
    while (i < j && iter++ < max_iter) {
        ...
        if (h_clusters)
            relax_ptp<<<NB(end - start), NT>>>(d_mesh, d_dist[!d], d_dist[d],
                                               d_clusters[!d], d_clusters[d],
                                               d_sorted, end, start);
        else
            relax_ptp<<<NB(end - start), NT>>>(d_mesh, d_dist[!d], d_dist[d],
                                               d_sorted, end, start);
        ...
        relative_error<<<NB(n_cond), NT>>>(d_error, d_dist[!d], d_dist[d],
                                           start, start + n_cond, d_sorted);
        cudaDeviceSynchronize();

        if (n_cond == thrust::count_if(thrust::device, d_error + start,
                                       d_error + start + n_cond, is_ok()))
            i++;
        if (j < limits.size() - 1)
            j++;
...
__global__ void relax_ptp(CHE*        mesh,
                          distance_t* new_dist,
                          distance_t* old_dist,
                          index_t*    sorted,
                          index_t     end,
...
__global__ void relax_ptp(CHE*        mesh,
                          distance_t* new_dist,
                          distance_t* old_dist,
                          index_t*    new_clusters,
                          index_t*    old_clusters,
                          index_t*    sorted,
                          index_t     end,
                          index_t     start)
{
    ...
    if (v < end) {
        ...
        if (v < mesh->n_vertices) {
            ...
            cu_for_star(he, mesh, v)
            {
                ...
                if (d < new_dist[v]) {
                    new_dist[v] = d;
                    new_clusters[v] = old_dist[mesh->VT[cu_prev(he)]] <
                                              old_dist[mesh->VT[cu_next(he)]] ?
                                          old_clusters[mesh->VT[cu_prev(he)]] :
                                          old_clusters[mesh->VT[cu_next(he)]];
                }
            }
        }
    }
...
__global__ void relative_error(distance_t*       error,
                               const distance_t* new_dist,
                               const distance_t* old_dist,
                               const index_t     start,
                               const index_t     end,
...
__forceinline__ __device__ distance_t cu_update_step(CHE*              mesh,
                                                     ...
                                                     const index_t&    he)
{
    ...
    distance_t dis = delta * delta - (Q[0][0] + Q[0][1] + Q[1][0] + Q[1][1]) *
                                         (t[0] * t[0] * Q[0][0] +
                                          t[0] * t[1] * (Q[1][0] + Q[0][1]) +
                                          t[1] * t[1] * Q[1][1] - 1);
    ...
    vertex_cu n(tp[0] * (X[0][0] * Q[0][0] + X[1][0] * Q[1][0]) +
                    tp[1] * (X[0][0] * Q[0][1] + X[1][0] * Q[1][1]),
                tp[0] * (X[0][1] * Q[0][0] + X[1][1] * Q[1][0]) +
                    tp[1] * (X[0][1] * Q[0][1] + X[1][1] * Q[1][1]),
                tp[0] * (X[0][2] * Q[0][0] + X[1][2] * Q[1][0]) +
                    tp[1] * (X[0][2] * Q[0][1] + X[1][2] * Q[1][1]));
    ...
    if (t[0] == INFINITY || t[1] == INFINITY || dis < 0 || c[0] >= 0 ||
        c[1] >= 0) {
        distance_t dp[2];
        dp[0] = dist[x[0]] + *X[0];
        dp[1] = dist[x[1]] + *X[1];
...
__host__ __device__ bool is_ok::operator()(const distance_t& val) const
{
    return val < PTP_TOL;
}
