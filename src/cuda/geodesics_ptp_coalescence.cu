...
double parallel_toplesets_propagation_coalescence_gpu(
    const ptp_out_t&       ptp_out,
    che*                   mesh,
    ...
    const bool&            set_inf)
{
    ...
    if (ptp_out.clusters) {
        ...
        d = run_ptp_coalescence_gpu(d_mesh, h_mesh->n_vertices, h_dist, d_dist,
                                    sources, {toplesets.limits, inv}, d_error,
...
index_t run_ptp_coalescence_gpu(CHE*                   d_mesh,
                                const index_t&         n_vertices,
                                distance_t*            h_dist,
                                distance_t**           d_dist,
                                const vector<index_t>& sources,
                                const toplesets_t&     inv,
                                distance_t*            d_error,
                                index_t*               h_clusters,
                                index_t**              d_clusters)
{
#pragma omp parallel for
    ...
    while (i < j && iter++ < max_iter) {
        ...
        if (h_clusters)
            relax_ptp_coalescence<<<NB(end - start), NT>>>(
                d_mesh, d_dist[!d], d_dist[d], d_clusters[!d], d_clusters[d],
                end, start);
        else
            relax_ptp_coalescence<<<NB(end - start), NT>>>(
                d_mesh, d_dist[!d], d_dist[d], end, start);
...
__global__ void relax_ptp_coalescence(CHE*        mesh,
                                      distance_t* new_dist,
                                      distance_t* old_dist,
                                      index_t     end,
                                      index_t     start)
