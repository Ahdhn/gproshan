...
void che::star(star_t& s, const index_t& v)
{
    ...
    for_star(he, this, v) s.push_back(he);
}
...
{
    for_border(he, this, BT[b]) border.push_back(VT[he]);
}
...
void che::flip(const index_t& e)
{
    ...
    if (EVT[va] == next(hb) || EVT[va] == ha)
        EVT[va] = prev(hb);
    if (EVT[vb] == next(ha) || EVT[vb] == hb)
        EVT[vb] = prev(ha);
    if (EVT[vc] == prev(ha))
        EVT[vc] = next(hb);
    if (EVT[vd] == prev(hb))
...
{
    index_t he = t * P;
    real_t  h[3] = {*(GT[VT[next(he)]] - GT[VT[he]]),
                   *(GT[VT[prev(he)]] - GT[VT[next(he)]]),
                   *(GT[VT[he]] - GT[VT[prev(he)]])};
    return (4 * sqrt(3) * area_trig(t)) /
...
{
    area_t area_star = 0;
    for_star(he, this, v) area_star += area_trig(trig(he));
...
{
    vertex n =
        (GT[VT[next(he)]] - GT[VT[he]]) * (GT[VT[prev(he)]] - GT[VT[he]]);
    return n / *n;
}
...
vertex che::corr_vertex(corr_t& corr) const
{
    ...
    return corr.alpha[0] * gt_vt(he) + corr.alpha[1] * gt_vt(next(he)) +
           corr.alpha[2] * gt_vt(prev(he));
}
...
{
    return sizeof(*this) + n_vertices_ * (sizeof(vertex) + sizeof(index_t)) +
           filename_.size() +
...
void che::set_vertices(const vertex* const& positions,
                       size_t               n,
                       const index_t&       v_i)
{
    if (!positions)
...
void che::compute_toplesets(index_t*&              toplesets,
                            index_t*&              sorted,
...
#pragma omp parallel for
    ...
    for (index_t e = 0; e < n_flips; e++)
        if (OT[ET[e]] == NIL || (normal_he(ET[e]), normal_he(OT[ET[e]])) <
                                    0.8)  // could be improve by quadric error
        {
            index_t he = ET[e];
...
void che::remove_non_manifold_vertices()
{
    for (index_t he = 0; he < n_half_edges_; he += 3) {
        if (EVT[VT[he]] == NIL || EVT[VT[he + 1]] == NIL ||
            EVT[VT[he + 2]] == NIL) {
            VT[he] = NIL;
            VT[he + 1] = NIL;
            VT[he + 2] = NIL;
    ...
    for (index_t v = removed[0] + 1; v < n_vertices_; v++) {
        if (v < removed[r]) {
            for_star(he, this, v) if (VT[he] != NIL) VT[he] = v - d;
        } else if (v == removed[r]) {
            d++;
            r++;
        }
    ...
    delete_me();
    gproshan_debug(removing vertex);
    init(new_vertices.data(), new_vertices.size(), new_faces.data(),
         new_faces.size() / P);
    gproshan_debug(removing vertex);
}
...
void che::merge(const che* mesh, const vector<index_t>& com_vertices)
{
    ...
    memcpy(aGT + n_vertices_, mesh->GT + ncv,
           sizeof(vertex) * (nv - n_vertices_));

    memcpy(aVT, VT, sizeof(index_t) * n_half_edges_);

    index_t* t_aVT = aVT + n_half_edges_;
    for (index_t he = 0; he < mesh->n_half_edges_; he++)
        t_aVT[he] = mesh->VT[he] < ncv ? com_vertices[mesh->VT[he]] :
                                         mesh->VT[he] + n_vertices_ - ncv;
    gproshan_debug(fill_holes);
    ...
    if (is_open)
        aEVT[com_vertices[0]] =
            mesh->EVT[0] != NIL ? mesh->EVT[0] + n_half_edges_ : NIL;

    gproshan_debug(fill_holes);
    index_t* t_aEVT = aEVT + n_vertices_;
    for (index_t v = ncv; v < mesh->n_vertices_; v++)
        t_aEVT[v - ncv] =
            mesh->EVT[v] != NIL ? mesh->EVT[v] + n_half_edges_ : NIL;
    gproshan_debug(fill_holes);
...
void che::set_head_vertices(index_t* head, const size_t& n)
{
    for (index_t v, i = 0; i < n; i++) {
        ...
        for_star(he, this, v) VT[he] = i;
        for_star(he, this, i) VT[he] = v;
...
void che::init(const vertex*  vertices,
               const index_t& n_v,
               const index_t* faces,
               const index_t& n_f)
{
