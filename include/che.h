...
#define for_star(he, mesh, v)                                       \
    for (index_t stop = mesh->evt(v), he = mesh->evt(v); he != NIL; \
         he = (he = mesh->ot(prev(he))) != stop ? he : NIL)
#define for_border(he, mesh, v)                                     \
    for (index_t stop = mesh->evt(v), he = mesh->evt(v); he != NIL; \
         he = (he = mesh->evt(mesh->vt(next(he)))) != stop ? he : NIL)
...
class che
{
   ...
   public:
    ...
    void               set_vertices(const vertex* const& positions,
                                    size_t               n = 0,
                                    const index_t&       v_i = 0);
    ...
    void               reload();
    void               compute_toplesets(index_t*&                   rings,
                                         index_t*&                   sorted,
   ...
   protected:
    void         delete_me();
    void         init(const vertex*  vertices,
                      const index_t& n_v,
                      const index_t* faces,
                      const index_t& n_f);
