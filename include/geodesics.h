...
/*!
    Compute the geodesics distances on a mesh from a source or multi-source.
   This class implements the Fast Marching algorithm without deal with obtuse
   triangles. Also, if the options PTP_CPU or PTP_GPU are enables, compute the
   geodesics distances executing the Parallel Toplesets Propagation algorithm.
*/
class geodesics
{
   public:
    enum option_t
    {
        FM,  ///< Execute Fast Marching algorithm
#ifdef GPROSHAN_CUDA
   ...
   private:
    ...
    index_t* sorted_index;  ///< Sort vertices by topological level or geodesic
                            ///< distance.
    const size_t& n_vertices;  ///< Number of vertices.
    size_t
         n_sorted;  ///< Number of vertices sorted by their geodesics distance.
    bool free_dist;

   public:
    geodesics(
        che* mesh,  ///< input mesh must be a triangular mesh.
        ...
        const bool&        cluster =
            false,  ///< to cluster vertices to closest source.
        const size_t&     n_iter = 0,  ///< maximum number of iterations.
        const distance_t& radio =
            INFINITY  ///< execute until the specific radio.
    );
   ...
   private:
    void execute(che*                        mesh,
                 ...
                 const distance_t&           radio,
                 const option_t&             opt);
    void run_fastmarching(che*                        mesh,
                          const std::vector<index_t>& sources,
                          const size_t&               n_iter,
                          const distance_t&           radio);
    void run_parallel_toplesets_propagation_cpu(
        che*                        mesh,
        const std::vector<index_t>& sources,
        const size_t&               n_iter,
        const distance_t&           radio);
